...
const generateRefreshToken = (userId, displayName, role) => {
  return jwt.sign({ userId, displayName, role }, process.env.REFRESH_TOKEN_SECRET, {
    expiresIn: "7d",
  });
};
...
async function refreshToken(req, res, next){
  try {
    ...
    if(!token){
      return res.status(401).json({ message: "Unauthorized, no refreshToken provided" });
    }
    const decodedToken = jwt.verify(token, process.env.REFRESH_TOKEN_SECRET, (err, decodedToken) => {
      if(err)
        return res.status(403).json({ message: "Forbidden, not recognized or invalid refreshToken" });
      ...
      // Si no se genera uno nuevo, el usuario tendrá que iniciar sesión de nuevo cuando expire el refreshToken
